...
:root{
  ...
  --success:     #4ade5f;   /* 通电成功（与线缆绿同值，避免系统里出现两个肉眼难辨的绿） */
...
直接复用 §2 线缆调色板的前 6 色，避免系统里维护两套近似色：

`#38e1ff` 青 ｜ `#ffb02e` 琥珀 ｜ `#ff4f6d` 红 ｜ `#4ade5f` 绿 ｜ `#b07bff` 紫 ｜ `#ff8ad8` 粉

> 头像色仅用于档案身份标识，**不出现在棋盘上**，因此与线缆色同值不会造成游戏内混淆。头像的辨识由「颜色 + 图标」共同承担（见 §6.1），本身已满足色盲可辨。
...
**权威数据源 = `levels.js` 的 `POWERLINK_PALETTE`。** 本表与之对齐，颜色值以实现为准。

| 索引 | 名称 | HEX | 形状 | 形状 ID | 何时出现 |
|---|---|---|---|---|---|
| 0 | 青 | `#38e1ff` | ● 圆 | `circle` | K≥1（永远出现） |
| 1 | 琥珀 | `#ffb02e` | ■ 方 | `square` | K≥2 |
| 2 | 红 | `#ff4f6d` | ▲ 三角 | `triangle` | K≥3 |
| 3 | 绿 | `#4ade5f` | ✚ 十字 | `cross` | K≥4 |
| 4 | 紫 | `#b07bff` | ⬢ 六边 | `hexagon` | K≥5 |
| 5 | 粉 | `#ff8ad8` | ◆ 菱形 | `diamond` | K≥6 |
| 6 | 黄 | `#f2ef7a` | ★ 星 | `star` | K=7（上限） |
| 7 | 松绿 | `#00c2a8` | ◎ 环 | `ring` | 备用，当前 `MAX_PAIRS=7` 故不启用 |

**关键配对校验（红绿色盲模拟核对）**

| 风险配对 | 色觉风险 | 形状区分 | 结论 |
|---|---|---|---|
| 0 青 / 3 绿 | deuteranopia 下两者均偏青灰，**收敛风险最高** | `圆` vs `十字` | ✓ 形状差异极大 |
| 1 琥珀 / 6 黄 | 同属黄色family，色相接近 | `方` vs `星` | ✓ |
| 2 红 / 5 粉 | protanopia 下可能靠近 | `三角` vs `菱形` | ✓ |
| 1 琥珀 / 5 粉 | 明度接近 | `方` vs `菱形` | △ 菱形=45°旋转的方形，但两色色相/明度均拉开，可接受 |
| 0 青 / 7 松绿 | 色相非常接近 | `圆` vs `环` | ⚠ 形状区分弱，但 `MAX_PAIRS=7` 下索引 7 不启用，**当前无影响** |

> ⚠️ **若未来把 `MAX_PAIRS` 提到 8**，必须先把索引 7 的 `#00c2a8` 换成远离青色的色相（建议改为深橙 `#e2703a` 或靛蓝 `#5b6cff`），否则「青圆 / 松绿环」在色盲 + 小尺寸下几乎不可分辨。

**工程实现要求**：颜色与形状必须**成对定义在同一个常量数组**（`POWERLINK_PALETTE`），禁止两处分别维护。UI 与 canvas 共用同一数组。

```js
var PALETTE = [
  { color: '#38e1ff', shape: 'circle',   label: '青'   },
  { color: '#ffb02e', shape: 'square',   label: '琥珀' },
  { color: '#ff4f6d', shape: 'triangle', label: '红'   },
  { color: '#4ade5f', shape: 'cross',    label: '绿'   },
  { color: '#b07bff', shape: 'hexagon',  label: '紫'   },
  { color: '#ff8ad8', shape: 'diamond',  label: '粉'   },
  { color: '#f2ef7a', shape: 'star',     label: '黄'   },
  { color: '#00c2a8', shape: 'ring',     label: '松绿' }
];
```

> **与当前实现的差异（需工程改 1 处）**：颜色值完全不变，仅**形状字段的排列**需调整为上表。当前实现为 `square/triangle/circle/diamond/hexagon/cross/star/ring`，存在两处色盲隐患：
> ① **索引 0「青-方」与索引 3「绿-菱」** —— 菱形是 45° 旋转的方形，在 `cell*0.19`（约 7px）尺寸下极易混淆，而青/绿正是 deuteranopia 下收敛风险最高的一对。且这两色从 **K≥4 起同时出现**，覆盖绝大多数关卡，影响面最大。
> ② 索引 1「琥珀-三角」与索引 6「黄-星」 —— 同为黄色系 + 同为尖角形状（仅 K=7 时共现，影响面较小）。
> 修正方式是**仅重排 `shape` 字段**，8 个形状 ID 与绘制代码完全不变，改动量 = 8 行内的字符串顺序，**无需新增任何绘制逻辑**。
